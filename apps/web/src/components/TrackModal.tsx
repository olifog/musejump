...
export const TrackModal = ({ track, onClose }: TrackModalProps) => {
  ...
  const { data: jumps, refetch: refetchJumps } = useQuery(
    trpc.jumps.getJumps.queryOptions(track ? {
      songId: track.id,
    } : skipToken)
  );
  ...
  return (
    ...
    >
      <div
        className="bg-gray-800 rounded-xl w-full max-w-md p-5 mx-4 relative max-h-[90vh] overflow-y-auto"
